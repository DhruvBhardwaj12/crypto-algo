"""Run MA crossover backtest on BTC and ETH with Indian cost model.

Run:
    uv run python scripts/backtest_ma.py
"""
...
def main() -> None:
    ...
    for symbol in symbols:
        path = data_dir / symbol / "1h" / "2023-01-01_2024-01-01.parquet"
        ...
        print(f"\n=== {symbol} — MA({FAST_WINDOW}/{SLOW_WINDOW}) ===")
        print(f"Final equity: {result.final_equity:,.2f} "
              f"(start {INITIAL_EQUITY:,.2f})")
        print(f"Buy & hold final: {bh.iloc[-1]:,.2f}  "
              f"(total return {bh_total_return:+.2%})")
    ...
    btc = load_parquet(data_dir / "BTCUSDT" / "1h" / "2023-01-01_2024-01-01.parquet")
    bh = buy_and_hold_equity(btc, INITIAL_EQUITY)
    ax.plot(bh.index, bh.values, label="BTC buy & hold", linestyle="--", alpha=0.7)

    ax.set_title(f"MA Crossover ({FAST_WINDOW}/{SLOW_WINDOW}) vs Buy & Hold — 2023")
    ...
    out_path = Path("research/backtest_ma.png")
